...
${i}--color-shadow: 0 0% 0%;${customVars(i)}`;
  ...
  return `/* ─────────────────────────────────────────────────────────
   Generated by structyl Theme Builder · ${new Date().toISOString().slice(0, 10)}
...
const THEME_PRESETS = [
  { name: 'Structyl',    primary: '#5754a3', destructive: '#d32f2f', success: '#2e7d32', warning: '#ed6c02', info: '#0288d1', radius: '0.5rem' },
  ...
  return (
    <Box className="space-y-2">
      {pairs.map(({ label, fg, bg }) => (
        <Box key={label} className="flex items-center justify-between gap-2">
          <Box className="flex min-w-0 items-center gap-2">
            <Box className="h-4 w-4 shrink-0 rounded border border-border" style={{ background: fg }} />
  ...
  return (
    <Box className="space-y-5">
      {(['sm', 'md', 'lg'] as const).map(size => {
        ...
        return (
          <Box key={size} className="space-y-2">
            <Box className="flex items-center justify-between">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">
                {size.toUpperCase()}
              </span>
              <Box className="h-6 w-10 rounded border border-border bg-card" style={{ boxShadow: shadowStr(def, size) }} />
            </Box>
            <Box className="grid grid-cols-3 gap-2 text-[9px] text-muted-foreground">
              {[
...
function FontPicker({ label, value, options, onChange }: {
  ...
  return (
    <Box className="space-y-1.5">
      <Typography as="p" variant="body2" className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">{label}</Typography>
      <Box className="flex flex-wrap gap-1.5">
        {options.map(o => (
          <Button variant="ghost" key={o.value} onClick={() => onChange(o.value)}
...
function Panel({ title, children, defaultOpen = true }: {
  ...
  return (
    <Box className="space-y-2.5 border-b border-border/40 pb-5">
      <Button variant="ghost" onClick={() => setOpen(o => !o)} className="flex w-full items-center justify-between">
        <Typography as="p" variant="body2" className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground/70">{title}</Typography>
        {open
...
function ColorRow({ label, hex, onChange, presets, onSnap }: {
  ...
  return (
    <Box className="space-y-1.5">
      {label && <Typography as="p" variant="body2" className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">{label}</Typography>}
      {presets && (
        <Box className="flex flex-wrap gap-1.5">
          {presets.map(a => (
            <Button variant="ghost" key={a.hex} onClick={() => { onSnap?.(); onChange(a.hex); }} title={a.name}
      ...
      <Box className="flex items-center gap-2">
        ...
        <Input size="sm" value={hex.toUpperCase()} onFocus={onSnap} onChange={e => onChange(e.target.value)}
...
> {
  ...
  override render() {
    if (this.state.error)
      return <Box className="flex h-full min-h-[80px] items-center justify-center">
...
/* Component card previews are shared with the docs gallery. */
const THEME_CARD_CONFIG = COMPONENT_CARD_PREVIEWS;
...
function ImportModal({ onClose, onImport }: {
  ...
  return (
    <Box className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <Box className="mx-4 w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-xl">
        <Typography as="h2" variant="h2" className="text-base font-semibold">Import CSS variables</Typography>
        <Typography as="p" variant="body2" className="mt-1 text-[12px] text-muted-foreground">
          Paste your CSS. Matching structyl tokens will be loaded into the builder.
        </Typography>
        ...
        <Box className="mt-4 flex justify-end gap-2">
          <Button variant="ghost" onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-[12px] text-muted-foreground hover:text-fg">Cancel</Button>
          <Button variant="ghost" onClick={() => { onImport(parseCSSVars(text)); onClose(); }} disabled={!text.trim()}
...
function LiveShowcase({ forcedMode }: { forcedMode?: 'light' | 'dark' }) {
  ...
  return (
    <Box className="overflow-hidden rounded-2xl border border-border bg-[hsl(var(--color-bg))] text-[hsl(var(--color-fg))] shadow-lg" style={vars}>
      {forcedMode ? (
        <Box className="flex items-center gap-1.5 border-b border-[hsl(var(--color-border))] px-3 py-1.5 text-[9px] font-semibold uppercase tracking-widest text-[hsl(var(--color-muted-fg))]">
          {forcedMode === 'light' ? <Sun className="h-2.5 w-2.5" /> : <Moon className="h-2.5 w-2.5" />}
          {forcedMode}
        </Box>
      ) : (
        <Box className="flex items-center gap-3 border-b border-[hsl(var(--color-border))] bg-[hsl(var(--color-card))] px-4 py-2.5">
          ...
          <Box className="flex flex-1 items-center gap-2 rounded-md border border-[hsl(var(--color-border))] bg-[hsl(var(--color-muted))]/40 px-2.5 py-1 text-[11px] text-[hsl(var(--color-muted-fg))]">
            <Monitor className="h-3 w-3" /> app.company.com
          </Box>
        </Box>
      )}
      <Box className="flex min-h-[380px]">
        <Box className="hidden w-[130px] shrink-0 border-r border-[hsl(var(--color-border))] bg-[hsl(var(--color-card))]/50 p-3 sm:block" style={vars}>
          <Typography as="p" variant="body2" className="mb-3 px-2 text-[9px] font-semibold uppercase tracking-widest text-[hsl(var(--color-muted-fg))]/60">Nav</Typography>
          {['Dashboard', 'Users', 'Settings', 'Reports'].map(item => (
            <Box key={item} className={`mb-0.5 rounded-lg px-2.5 py-1.5 text-[11px] font-medium ${item === 'Users' ? 'bg-primary/10 text-primary' : 'text-[hsl(var(--color-muted-fg))]'}`}>
              {item}
            </Box>
          ))}
          <Box className="mt-3 rounded-lg border border-destructive/20 bg-destructive/5 px-2 py-1 text-[10px] text-destructive">3 alerts</Box>
        </Box>
        <Box className="flex-1 space-y-4 p-4" style={vars}>
          <Box className="flex items-center justify-between">
            <Box>
              <Typography as="h3" variant="h3" className="text-sm font-semibold text-[hsl(var(--color-fg))]">Users</Typography>
              <Typography as="p" variant="body2" className="text-[11px] text-[hsl(var(--color-muted-fg))]">Manage your team</Typography>
            </Box>
            <Box className="flex gap-2">
              <Button variant="ghost" className="rounded-lg border border-[hsl(var(--color-border))] px-3 py-1.5 text-[11px] text-[hsl(var(--color-muted-fg))]">Export</Button>
              <Button variant="ghost" className="rounded-lg bg-primary px-3 py-1.5 text-[11px] font-medium text-primary-foreground">+ Invite</Button>
            </Box>
          </Box>
          <Box className="grid grid-cols-3 gap-3">
            {[['Total', '2,491', '+12%', true], ['Active', '148', '+3%', true], ['Issues', '3', '-62%', false]].map(([lbl, v, ch, up]) => (
              <Box key={String(lbl)} className="rounded-xl border border-[hsl(var(--color-border))] bg-[hsl(var(--color-card))] p-3" style={vars}>
                <Typography as="p" variant="body2" className="text-[10px] text-[hsl(var(--color-muted-fg))]">{lbl}</Typography>
                <Typography as="p" variant="body2" className="mt-1 text-lg font-bold text-[hsl(var(--color-fg))]">{v}</Typography>
                <Typography as="p" variant="body2" className={`text-[10px] font-medium ${up ? 'text-success' : 'text-destructive'}`}>{String(ch)}</Typography>
              </Box>
            ))}
          </Box>
          <Box className="flex gap-0.5 border-b border-[hsl(var(--color-border))]">
          ...
          <Box className="overflow-hidden rounded-xl border border-[hsl(var(--color-border))]">
            <Box className="grid grid-cols-4 border-b border-[hsl(var(--color-border))] bg-[hsl(var(--color-muted))]/30 px-3 py-2 text-[9px] font-semibold uppercase tracking-wider text-[hsl(var(--color-muted-fg))]">
            ...
            {[['Alice Chen','Admin','Active',true],['Bob Smith','Editor','Away',false],['Carol Wu','Viewer','Active',true]].map(([nm,role,st,active]) => (
              <Box key={String(nm)} className="grid grid-cols-4 items-center border-t border-[hsl(var(--color-border))]/60 px-3 py-2.5 text-[11px]">
                <Box className="flex items-center gap-2">
                  <Box className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 text-[9px] font-bold text-primary">{String(nm)[0]}</Box>
                ...
                <Button variant="ghost" className="text-right text-[10px] text-primary">Edit</Button>
              </Box>
            ))}
          </Box>
          <Box className="flex items-center gap-2 rounded-xl border border-[hsl(var(--color-border))] p-3">
            <Box onClick={() => setChecked(c => !c)}
          ...
          <Box className="flex items-start gap-2 rounded-xl border border-warning/30 bg-warning/5 px-3 py-2.5">
            <Box className="mt-0.5 h-2 w-2 shrink-0 rounded-full bg-warning" />
            <Typography as="p" variant="body2" className="text-[11px] text-warning">3 users have pending invitations expiring today.</Typography>
          </Box>
        </Box>
      </Box>
    </Box>
  ...
  return (
    <Box className="bg-bg text-fg min-h-screen">
      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-bg/80 backdrop-blur-md">
        <Box className="mx-auto flex h-[52px] max-w-[1500px] items-center gap-3 px-6">
          <Link href="/" className="flex items-center">
            <img src="/logo-light.svg" alt="structyl" className="h-7 dark:hidden" />
            <img src="/logo.svg"       alt="structyl" className="hidden h-7 dark:block" />
          </Link>
          ...
          <Box className="ml-auto flex items-center gap-2">
            ...
            <Button variant="ghost" onClick={() => makeDownload(css, 'structyl-theme.css', 'text/css')}
      ...
      <Box className="mx-auto grid max-w-[1500px] grid-cols-1 lg:grid-cols-[1fr_320px]">

        {/* ── Left: showcase ──────────────────────────────────────── */}
        <main className="min-w-0 px-6 py-8 lg:px-10">
          <Box className="mb-6 flex items-start justify-between gap-4 flex-wrap">
            <Box>
              <Typography as="h1" variant="h1" className="text-2xl font-semibold tracking-tight">Theme Builder</Typography>
              <Typography as="p" variant="body2" className="mt-1 text-sm text-muted-foreground">Configure on the right — every component updates live.</Typography>
            </Box>
            <Button variant="ghost" onClick={() => setSplitView(s => !s)}
          ...
          <section className="mb-10">
            <Typography as="p" variant="body2" className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">Live preview</Typography>
            {splitView ? (
              ...
              return (
                <section key={cat}>
                  <Box className="mb-3 flex items-center gap-3">
                    <Typography as="h2" variant="h2" className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">{cat}</Typography>
                    <span className="text-[10px] text-muted-foreground/40">{items.length}</span>
                    <Box className="h-px flex-1 bg-border/40" />
                  </Box>
                  <Box className="grid grid-cols-1 gap-5">
                    {items.map(c => {
                      const config = THEME_CARD_CONFIG[c.slug];
                      const minH = config?.height ?? 180;
                      const preview = config?.preview ?? c.preview;
                      return (
                        <Box key={c.slug} className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all hover:shadow-md">
                          <Box className="flex items-center justify-between border-b border-border/50 px-4 py-3">
                          ...
                          <Box className="flex flex-1 items-center justify-center overflow-hidden p-6" style={{ minHeight: minH }}>
                            <Box className="flex w-full min-w-0 max-w-2xl items-center justify-center [&>*]:max-w-full">
          ...
          <Typography as="p" variant="body2" className="mt-8 text-center text-[11px] text-muted-foreground/40">
        ...
        <aside className="sticky top-[52px] hidden h-[calc(100vh-52px)] overflow-y-auto border-l border-border/50 bg-card/30 lg:block">
          <Box className="space-y-0 px-5 py-5">

            <Panel title="Theme presets">
              <Box className="grid grid-cols-4 gap-1.5">
                {THEME_PRESETS.map(preset => (
                  <Button variant="ghost" key={preset.name} onClick={() => applyPreset(preset)} title={preset.name}
                    className="group flex h-auto flex-col items-center gap-1.5 rounded-xl border border-border p-2.5 text-[9px] font-medium transition-all hover:border-primary hover:bg-accent">
                    <Box
            ...
            <Panel title="Custom colors" defaultOpen={false}>
              {customColors.length > 0 && (
                <Box className="mb-2 space-y-2">
                  {customColors.map(c => (
                    <Box key={c.id} className="flex items-center gap-2">
                      <Box className="h-5 w-5 shrink-0 rounded-full border border-border" style={{ background: c.hex }} />
                      ...
                      <Button variant="ghost" onClick={() => setCustomColors(prev => prev.filter(x => x.id !== c.id))}
              ...
              {addingColor ? (
                <Box className="space-y-2 rounded-xl border border-border p-3">
                  <Input size="sm" value={newColorName} onChange={e => setNewColorName(e.target.value)}
                  ...
                  <Box className="flex gap-2">
                    ...
                    <Button variant="ghost" onClick={addCustomColor} disabled={!newColorName.trim()}
            ...
            <Panel title="Appearance">
              <Box className="grid grid-cols-3 gap-1.5">
                {([['light', Sun], ['dark', Moon], ['system', Monitor]] as const).map(([m, Icon]) => (
                  <Button variant="ghost" key={m} onClick={() => setMode(m === 'system' ? 'system' : m)}
            ...
            <Panel title="Border radius">
              <Box className="grid grid-cols-4 gap-1.5">
                {RADII.map(r => (
                  <Button variant="ghost" key={r.v} onClick={() => { snap(); setRadius(r.v); }}
                    className={`flex h-auto flex-col items-center gap-1.5 rounded-lg border px-1 py-2.5 text-[9px] font-medium ${
            ...
            <Panel title="Font scale">
              <Box className="grid grid-cols-5 gap-1.5">
                {SCALES.map(s => (
                  <Button variant="ghost" key={s.v} onClick={() => { snap(); setScale(s.v); }}
            ...
            <Panel title="Density" defaultOpen={false}>
              <Box className="grid grid-cols-3 gap-1.5">
                {DENSITY_OPTIONS.map(d => (
                  <Button variant="ghost" key={d.v} onClick={() => { snap(); setDensity(d.v); }}
              ...
              <Typography as="p" variant="body2" className="text-[10px] text-muted-foreground/60">
            ...
            <Panel title="Border width" defaultOpen={false}>
              <Box className="grid grid-cols-3 gap-1.5">
                {BORDER_WIDTH_OPTIONS.map(b => (
                  <Button variant="ghost" key={b.v} onClick={() => { snap(); setBorderWidth(b.v); }}
                    className={`rounded-lg border py-2 text-[11px] font-medium ${
            ...
            <Panel title="Animations" defaultOpen={false}>
              <Box className="space-y-3">
                <Box>
                  <Typography as="p" variant="body2" className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">Easing</Typography>
                  <Box className="grid grid-cols-2 gap-1.5">
                    {EASING_OPTIONS.map(e => (
                      <Button variant="ghost" key={e.value} onClick={() => { snap(); setEasing(e.value); }}
                ...
                <Box>
                  <Typography as="p" variant="body2" className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">Duration preset</Typography>
                  <Box className="grid grid-cols-3 gap-1.5">
                    {DURATION_PRESETS.map(d => (
                      <Button variant="ghost" key={d.name} onClick={() => { snap(); setDurations({ fast: d.fast, normal: d.normal, slow: d.slow }); }}
                        className={`rounded-lg border py-1.5 text-[10px] font-medium ${
                  ...
                  <Typography as="p" variant="body2" className="mt-1 text-[10px] text-muted-foreground/60">{durations.fast}ms / {durations.normal}ms / {durations.slow}ms</Typography>
                </Box>
              </Box>
            </Panel>

            <Panel title="Generated CSS" defaultOpen={false}>
              <Button variant="ghost" onClick={() => setCssOpen(o => !o)}
            ...
            <Panel title="Tailwind config" defaultOpen={false}>
              <Button variant="ghost" onClick={() => setTwOpen(o => !o)}
                className="flex w-full items-center justify-between rounded-lg border border-border px-3 py-2 text-[11px] text-muted-foreground hover:text-fg">
              ...
              {twOpen && (
                <>
                  <Box className="max-h-[260px] overflow-y-auto">
                    <CodeBlock code={twConfig} lang="tsx" />
                  </Box>
                  <Button variant="ghost" onClick={() => makeDownload(twConfig, 'tailwind.config.js', 'text/javascript')}
          ...
          {/* Sticky footer */}
          <Box className="sticky bottom-0 flex gap-2 border-t border-border bg-card/95 px-5 py-3 backdrop-blur-md">
            ...
            <Button variant="outline" size="sm" onClick={() => makeDownload(css, 'structyl-theme.css', 'text/css')} className="gap-1.5 px-3">
              <Download className="h-3.5 w-3.5" />
            </Button>
          </Box>
        </aside>
