...
export const API: Record<string, ApiPart[]> = {
  ...
  select: [
    {
      ...
      props: [
        ...
        { name: 'name', type: 'string', description: 'Name for form submission.' },
        { name: 'dir', type: "'ltr' | 'rtl'", description: 'Reading direction.' },
        { name: 'searchable', type: 'boolean', default: 'false', description: 'Enable option search.' },
        { name: 'searchValue', type: 'string', description: 'Controlled search value.' },
        { name: 'onSearchValueChange', type: '(value: string) => void', description: 'Called when search text changes.' },
        { name: 'filterOption', type: '(option, searchValue) => boolean', description: 'Customize option matching.' },
        { name: 'onCreateOption', type: '(value: string) => void', description: 'Called when a new option is created.' },
        { name: 'createOptionLabel', type: 'ReactNode | (value) => ReactNode', description: 'Customize the create option label.' },
        { name: 'resetSearchOnClose', type: 'boolean', default: 'true', description: 'Clear search when the popup closes.' },
      ],
    },
    ...
    { name: 'Select.SearchInput', description: 'Search field shown when the root is searchable.', props: [className] },
    {
      name: 'Select.Content',
      ...
      props: [
        ...
        { name: 'side', type: "'top' | 'right' | 'bottom' | 'left'", description: 'Preferred side.' },
        { name: 'align', type: "'start' | 'center' | 'end'", description: 'Alignment.' },
        { name: 'options', type: 'SelectOption[]', description: 'Array-backed options rendered with windowing.' },
        { name: 'optionHeight', type: 'number', default: '36', description: 'Virtual row height in pixels.' },
        { name: 'optionOverscan', type: 'number', default: '6', description: 'Extra rows rendered above and below the viewport.' },
      ],
    },
    {
      name: 'Select.Item',
    ...
    { name: 'Select.CreateItem', description: 'Creates the current search text with `onCreateOption`.', props: [className] },
    { name: 'Select.Options', description: 'Virtualized renderer for large array-backed option lists.', props: [{ name: 'options', type: 'SelectOption[]', description: 'Options to render.' }, { name: 'itemHeight', type: 'number', default: '36', description: 'Row height in pixels.' }, { name: 'overscan', type: 'number', default: '6', description: 'Extra rows rendered outside the viewport.' }] },
  ...
  'multi-select': [
    {
      ...
      props: [
        ...
        { name: 'onValueChange', type: '(value: string[]) => void', description: 'Called when selected values change.' },
        { name: 'open', type: 'boolean', description: 'Controlled open state.' },
        { name: 'onOpenChange', type: '(open: boolean) => void', description: 'Called when open state changes.' },
        { name: 'disabled', type: 'boolean', default: 'false', description: 'Disable the multi-select.' },
        { name: 'required', type: 'boolean', default: 'false', description: 'Mark as required.' },
        { name: 'name', type: 'string', description: 'Name for form submission.' },
        { name: 'searchable', type: 'boolean', default: 'false', description: 'Enable option search.' },
        { name: 'onCreateOption', type: '(value: string) => void', description: 'Called when a new option is created.' },
    ...
    { name: 'MultiSelect.Trigger', description: 'The button that opens the listbox.', props: [className] },
    { name: 'MultiSelect.Value', description: 'Displays selected values inside the trigger.', props: [{ name: 'placeholder', type: 'ReactNode', description: 'Shown when no value is selected.' }, { name: 'options', type: 'MultiSelectOption[]', description: 'Used to resolve selected value labels.' }, { name: 'maxVisible', type: 'number', description: 'Styled wrapper limit before showing an overflow count.' }] },
    { name: 'MultiSelect.SearchInput', description: 'Search field shown when the root is searchable.', props: [className] },
    {
      name: 'MultiSelect.Content',
      description: 'The multi-selectable listbox popup.',
      props: [
        { name: 'options', type: 'MultiSelectOption[]', description: 'Array-backed options rendered with windowing.' },
        { name: 'optionHeight', type: 'number', default: '36', description: 'Virtual row height in pixels.' },
        { name: 'optionOverscan', type: 'number', default: '6', description: 'Extra rows rendered above and below the viewport.' },
      ],
    },
    { name: 'MultiSelect.Item', description: 'A toggleable option.', props: [{ name: 'value', type: 'string', description: 'The value toggled when selected.' }, { name: 'disabled', type: 'boolean', default: 'false', description: 'Disable the option.' }, { name: 'textValue', type: 'string', description: 'Override the search text.' }] },
    { name: 'MultiSelect.CreateItem', description: 'Creates and selects the current search text with `onCreateOption`.', props: [className] },
    { name: 'MultiSelect.Options', description: 'Virtualized renderer for large array-backed option lists.', props: [{ name: 'options', type: 'MultiSelectOption[]', description: 'Options to render.' }, { name: 'itemHeight', type: 'number', default: '36', description: 'Row height in pixels.' }, { name: 'overscan', type: 'number', default: '6', description: 'Extra rows rendered outside the viewport.' }] },
  ...
  'date-picker': [
    {
      ...
      props: [
        { name: 'value / defaultValue', type: 'Date | null', description: 'Controlled or uncontrolled date value.' },
        { name: 'onChange / onAccept', type: '(value, context) => void', description: 'MUI-style callbacks with validationError, source and shortcut metadata.' },
        { name: 'onError', type: '(error, value) => void', description: 'Called when validation error state changes.' },
        { name: 'open / onOpen / onClose', type: 'boolean / callbacks', description: 'Controlled popover state and lifecycle callbacks.' },
        { name: 'label / name / inputRef', type: 'React.ReactNode / string / ref', description: 'Field label and form integration props.' },
        { name: 'format / formatDensity', type: 'string | Intl.DateTimeFormatOptions', description: 'Input display format and dense/spacious separator density.' },
        { name: 'minDate / maxDate / disablePast / disableFuture', type: 'Date / boolean', description: 'Date validation bounds.' },
        { name: 'shouldDisableDate / shouldDisableMonth / shouldDisableYear', type: '(date: Date) => boolean', description: 'Custom date, month and year validation callbacks.' },
        { name: 'views / view / openTo / onViewChange', type: "Array<'day' | 'month' | 'year'>", description: 'MUI-compatible view control props.' },
        { name: 'selectedSections / onSelectedSectionsChange', type: 'FieldSelectedSections', description: 'MUI-compatible field section selection props.' },
        { name: 'loading / renderLoading', type: 'boolean / () => ReactNode', description: 'Render a loading state instead of the calendar.' },
        { name: 'dayOfWeekFormatter / displayWeekNumber / fixedWeekNumber / showDaysOutsideCurrentMonth', type: 'calendar props', description: 'Calendar rendering options.' },
        { name: 'disableOpenPicker / disabled / readOnly / autoFocus', type: 'boolean', description: 'Interaction and field state props.' },
        { name: 'desktopModeMediaQuery / orientation / reduceAnimations / referenceDate / timezone', type: 'MUI compatibility props', description: 'Accepted for API parity.' },
        { name: 'slots / slotProps / sx / viewRenderers', type: 'object', description: 'MUI customization props accepted by the component surface.' },
      ],
    },
    { name: 'DatePicker.Root', description: 'Compound date input — Calendar inside a Popover.', props: [{ name: 'value', type: 'Date | null', description: 'Controlled date.' }, { name: 'onValueChange', type: '(date?: Date) => void', description: 'Aura-style change callback.' }] },
    { name: 'DatePicker.Trigger', description: 'The button that opens the calendar.', props: [] },
    { name: 'DatePicker.Content', description: 'The popover containing the calendar.', props: [{ name: 'children', type: 'React.ReactNode', description: 'Custom calendar content. Defaults to DatePicker.Calendar.' }] },
    { name: 'DatePicker.Calendar', description: 'Styled calendar bound to DatePicker state.', props: [] },
  ],
  'color-picker': [
    {
  ...
  'time-picker': [
    {
      name: 'TimePicker',
      description: 'MUI-style time field with popover columns and segmented primitive parts.',
      props: [
        { name: 'value / defaultValue', type: 'Date | TimeValue | null', description: 'Controlled or uncontrolled time.' },
        { name: 'onChange / onAccept', type: '(value, context) => void', description: 'MUI-style callbacks with validation context.' },
        { name: 'ampm / ampmInClock', type: 'boolean', description: '12-hour mode and AM/PM display compatibility.' },
        { name: 'minTime / maxTime / disablePast / disableFuture', type: 'Date / boolean', description: 'Time validation bounds.' },
        { name: 'minutesStep / timeSteps', type: 'number / { hours, minutes, seconds }', description: 'Time option stepping.' },
        { name: 'shouldDisableTime / skipDisabled', type: '(value, view) => boolean', description: 'Disable or hide specific time options.' },
        { name: 'views / view / openTo / onViewChange', type: "Array<'hours' | 'minutes' | 'seconds'>", description: 'MUI-compatible time view props.' },
        { name: 'format / formatDensity / selectedSections', type: 'MUI field props', description: 'Formatting and field section props.' },
        { name: 'disableOpenPicker / disabled / readOnly / autoFocus', type: 'boolean', description: 'Interaction and field state props.' },
        { name: 'slots / slotProps / sx / viewRenderers / timezone', type: 'object / string', description: 'MUI customization and timezone compatibility props.' },
      ],
    },
    { name: 'TimePicker.Root', description: 'Compound segmented time input.', props: [{ name: 'value', type: 'Date | TimeValue | null', description: 'Controlled time.' }, { name: 'onValueChange', type: '(value: TimeValue) => void', description: 'Aura-style segment callback.' }] },
  ...
  'date-range-picker': [
    {
      ...
      props: [
        { name: 'value / defaultValue', type: '[Date | null, Date | null] | { from?: Date; to?: Date }', description: 'Controlled or uncontrolled range.' },
        { name: 'onChange / onAccept', type: '(range, context) => void', description: 'MUI-style callbacks with validation context.' },
        { name: 'shortcuts', type: 'DateRangePickerShortcut[] | false', description: 'Preset action buttons. Defaults include Today, Yesterday, Last 7 days, Last 30 days, This month and Last month.' },
        { name: 'calendars / currentMonthCalendarPosition', type: '1 | 2 | 3', description: 'Visible month count and current month placement compatibility.' },
        { name: 'rangePosition / defaultRangePosition / onRangePositionChange', type: "'start' | 'end'", description: 'Controlled edited edge of the range.' },
        { name: 'minDate / maxDate / disablePast / disableFuture', type: 'Date / boolean', description: 'Date validation bounds.' },
        { name: 'shouldDisableDate', type: "(date: Date, position: 'start' | 'end') => boolean", description: 'Custom disabled-date predicate with range edge.' },
        { name: 'disableAutoMonthSwitching / disableDragEditing', type: 'boolean', description: 'MUI range behavior compatibility props.' },
        { name: 'format / formatDensity / selectedSections', type: 'MUI field props', description: 'Formatting and field section props.' },
        { name: 'loading / renderLoading', type: 'boolean / () => ReactNode', description: 'Render loading state.' },
        { name: 'displayWeekNumber / fixedWeekNumber / showDaysOutsideCurrentMonth', type: 'calendar props', description: 'Calendar rendering options.' },
        { name: 'slots / slotProps / sx / viewRenderers / timezone', type: 'object / string', description: 'MUI customization and timezone compatibility props.' },
      ],
    },
    { name: 'DateRangePicker.Root', description: 'Compound date-range input — Calendar in a Popover.', props: [{ name: 'value', type: '{ from?: Date; to?: Date } | [Date | null, Date | null]', description: 'Controlled range.' }, { name: 'onValueChange', type: '(range) => void', description: 'Aura-style change callback.' }] },
    { name: 'DateRangePicker.Trigger', description: 'Opens the calendar.', props: [] },
    { name: 'DateRangePicker.Content', description: 'The popover with the range calendar.', props: [{ name: 'calendars', type: '1 | 2', description: 'Number of visible months.' }] },
    { name: 'DateRangePicker.Calendar', description: 'Styled range calendar bound to DateRangePicker state.', props: [] },
  ],
  'date-time-picker': [
    {
      name: 'DateTimePicker',
      description: 'MUI-style date-time field combining DatePicker and TimePicker props.',
      props: [
        { name: 'value / defaultValue', type: 'Date | null', description: 'Controlled or uncontrolled date-time value.' },
        { name: 'onChange / onAccept', type: '(value, context) => void', description: 'MUI-style callbacks with validation context.' },
        { name: 'minDateTime / maxDateTime', type: 'Date', description: 'Absolute date-time validation bounds.' },
        { name: 'minDate / maxDate / minTime / maxTime', type: 'Date', description: 'Separate date and time validation bounds.' },
        { name: 'ampm / minutesStep / timeSteps', type: 'boolean / number / object', description: 'Time selection behavior.' },
        { name: 'shouldDisableDate / shouldDisableMonth / shouldDisableYear / shouldDisableTime', type: 'validation callbacks', description: 'Custom validation callbacks.' },
        { name: 'views / view / openTo / onViewChange', type: 'date-time views', description: 'MUI-compatible view control props.' },
        { name: 'loading / renderLoading / format / formatDensity', type: 'MUI field props', description: 'Display and loading props.' },
        { name: 'slots / slotProps / sx / viewRenderers / timezone', type: 'object / string', description: 'MUI customization and timezone compatibility props.' },
      ],
    },
    { name: 'DateTimePicker.Root', description: 'Compound date-time input — Calendar and time segments inside a Popover.', props: [{ name: 'value', type: 'Date | null', description: 'Controlled date-time.' }] },
    { name: 'DateTimePicker.Trigger', description: 'Opens the date-time popover.', props: [] },
    { name: 'DateTimePicker.Content', description: 'The popover with calendar and time controls.', props: [] },
    { name: 'DateTimePicker.Calendar', description: 'Styled calendar bound to DateTimePicker state.', props: [] },
    { name: 'DateTimePicker.Segment', description: 'Time segment bound to DateTimePicker state.', props: [{ name: 'segment', type: "'hour' | 'minute' | 'second' | 'period'", description: 'Time unit to edit.' }] },
  ],
  'data-table': [
    {
      name: 'DataTable',
