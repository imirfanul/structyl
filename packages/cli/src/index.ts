...
program
  .name('your-lib')
  .description('CLI for adding @your-lib components to your project')
  .version('0.0.1');

program
  .command('init')
  .description('Initialize your-lib in your project')
  .action(async () => {
    ...
    const spinner = ora('Setting up your-lib').start();
    try {
      await fs.mkdir(answers.dir, { recursive: true });
      await fs.writeFile(
        'your-lib.config.json',
        JSON.stringify(
      ...
      console.log(kleur.dim(`\n  Config written to ${kleur.cyan('your-lib.config.json')}`));
      console.log(kleur.dim(`  Add components with ${kleur.cyan('npx your-lib add <name>')}`));
      console.log(kleur.dim(`  List components with ${kleur.cyan('npx your-lib list')}\n`));
    } catch (err) {
      spinner.fail('Failed');
      console.error(err);
      process.exit(1);
...
program
  ...
  .option('--source <path>', 'Path to @your-lib/styled/src for inlining')
  .action(async (components: string[], opts: { cwd: string; source?: string }) => {
    const configPath = path.join(opts.cwd, 'your-lib.config.json');
    const configRaw = await fs.readFile(configPath, 'utf8').catch(() => null);
    if (!configRaw) {
      console.error(kleur.red('No your-lib.config.json found. Run `npx your-lib init` first.'));
      process.exit(1);
    }
    ...
    const sourceRoot =
      opts.source ?? path.resolve(opts.cwd, 'node_modules/@your-lib/styled/src');
    ...
    try {
      for (const name of toCopy) {
        ...
        try {
          ...
          await fs.writeFile(
            dest,
            `// ${name} component scaffold (source not found at ${src})\n// Run with --source to point at @your-lib/styled/src.\n`,
          );
